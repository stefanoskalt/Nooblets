...
function render() {
    ...
    for (i = 0; i < keyFrameAnimations.length; i += 1) {
        ...
        var animationIndex = getIndexOfPropertyInArr(animationsInfo, "name", keyFrameAnimations[i].root.name);
        if (animationIndex != -1) {
            var animationClip = animationsInfo[animationIndex];
            var clipNumber = animationClip.clipPlaying;
            //increase precision of animation clips by allowing margin of error, of 0.030 seconds
            var timeDif = keyFrameAnimations[i].currentTime - animationClip.clips[clipNumber][1];
            if (timeDif.toFixed(3) > 0.030 * -1) {
                ...
                if (clipNumber == animationClip.clips.length - 1) {
                    animationsInfo[animationIndex].clipPlaying = 0;
                } else {
                    animationsInfo[animationIndex].clipPlaying = clipNumber + 1;
                }
                flag = 1;
            }
        } else if (keyFrameAnimations[i].currentTime.toFixed(1) === keyFrameAnimations[i].endTime.toFixed(1) &&
...
function startAnimations(mode, name) {
    ...
    if (mode === "play" && !keyFrameAnimations[i].isPlaying && keyFrameAnimations[i].currentTime === 0) {
        ...
        if (keyFrameAnimations[i].root.name == "Light Switch") {
...
function showMessage(obj) {
    ...
    if (obj.type == "description") {
        ...
        var conversationText = document.getElementById('conversationText');
        conversationText.innerHTML = obj.message;
        conversationText.style.fontStyle = "normal";
        ...
        var conversationText = document.getElementById('conversationText');
        conversationText.innerHTML = '"' + obj.message + '"';
        conversationText.style.fontStyle = "italic";
    }
...
function checkConversation(obj) {
    ...
    if (checkPerson(xmlConversations, name)) {
        for (var i = 0; i < modelsInteraction.length; i++) {
            var hasConversation = getIndexOfPropertyInArr(modelsConversation, "uniqueID", modelsInteraction[i].uniqueID);
            if (modelsInteraction[i].interactedByUser == true && hasConversation != -1) {
                ...
                if (modelsInteraction[i].conversationRead == false) {
                    var dialogue = getDialogue(xmlConversations, name, modelsInteraction[i].uniqueID);
                    modelsInteraction[i].conversationRead = true;
                    objToShowConvo.message = dialogue;
                    console.log(objToShowConvo);
                    break;
                    //dialogues.children[j].children[2].nodeValue == readStatus;
                    /*var allDialogues = getListOfDialogueNames(xmlConversations, name);
                    for (var j = 0; j < allDialogues.length; j++){
                        console.log("lol");
                        if ()
                    }*/
                } else if (modelsInteraction[i].conversationRead == true && hasConversation != -1) {
                    count += 1;
                }
                //all clues conversations were read
                if (count == modelsConversation.length) {
...
function checkInteraction() {
    ...
    if (localIntersections.length > 0) {
        if (localIntersections[0].distance < INTERACTION_DISTANCE_TRIGGER) {
            ...
            var interactionIndex = getIndexOfPropertyInArr(modelsInteraction, "uniqueID", parentModelName);
            ...
            var animationIndex = getIndexOfPropertyInArr(animationsInfo, "name", parentModelName);
            if (animationIndex != -1) {
...
function checkDescription() {
    ...
    if (intersectionsInteraction.length > 0) {
        if (intersectionsInteraction[0].distance < DESCRIPTION_DISTANCE_TRIGGER) {
            ...
            getDialogue(xmlConversations, "Police Officer", "TV");
            var obj = {
                "type": "description",
    ...
    myColladaLoader.load(toLoadModelArray.modelPath, function(collada) {
        ...
        if (toLoadModelArray.animation) {
            ...
            if (toLoadModelArray.animation.hasOwnProperty('clips')) {
                var obj = {
                    "name": toLoadModelArray.name,
                    "clips": toLoadModelArray.animation.clips,
                    "clipPlaying": 0
                }
