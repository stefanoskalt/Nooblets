...
function render() {
    ...
    for (i = 0; i < keyFrameAnimations.length; i += 1) {
        ...
        var animationIndex = getIndexOfPropertyInArr(animationsInfo, keyFrameAnimations[i].root.name);
        if (animationIndex != -1) {
            var animationClip = animationsInfo[animationIndex];
            var clipNumber = animationClip.clipPlaying;
            if (keyFrameAnimations[i].currentTime.toFixed(2) === animationClip.clips[clipNumber][1].toFixed(2)){
                console.log(animationClip.clipPlaying);
...
//retrieve model by name from a global array (e.g modelsCollision etc.)
function getIndexOfPropertyInArr(arr, name) {
    "use strict";
    var i;
    for (i = 0; i < arr.length; i += 1) {
        if (arr[i].name === name) {
